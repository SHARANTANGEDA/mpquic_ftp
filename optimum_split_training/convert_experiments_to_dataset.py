...
for idx, case in enumerate(samples):
    avg_transfer_time, avail_cnt, excep_cnt, split_ratio = 0, 0, 0, 0
    for i in range(0, runs_per_combination):
        ...
        try:
            ...
            avail_cnt += 1
            split_ratio = content[4].split(":")[1].strip()
        except:
    ...
    split_ratios_list.append(split_ratio)
...
results_df['split_ratio'], results_df['transfer_time'], results_df['scheduler'] = split_ratios_list, tr_col, scheduler_col
